...
##############check the first file  in /tmp/$IP/ on hdfs###############
      ...
      echo copy $IP:$file to local:$file
      echo "copy $IP:$file to local:$file" >> /tmp/$IP/log
	 /usr/local/hadoop-0.20.1-dev/bin/hadoop jar  /usr/local/hadoop-0.20.1-dev/bin/test_hdfs_IO.jar test_hdfs_IO read $file $file
         dif=""
         #dif=$(diff $file /tmp/$IP/test.iso | awk '{print $6}')
##############match the two file  ###############
if  [ -z $dif ] ; then
