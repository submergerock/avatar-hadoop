...
public class AvatarDataNode extends DataNode {
  ...
  public static  JvmInfo jvmInfo = new JvmInfo();
  ...
  void startDataNode(Configuration conf, 
	...
	LOG.info("wzt debuginfo:----------AvatarDataNode.startDataNode()-------000-----version: 1--");
    ...
    //LOG.info("----------AvatarDataNode.startDataNode()-------111-----version: 1--");    
	...
	LOG.info("wzt debuginfo:----------AvatarDataNode.startDataNode()-------get conf complete-----version: 1--");    
    String address = 
	...
	LOG.info("wzt debuginfo:----------AvatarDataNode.startDataNode()-------dn create socket port and init datastorage,dnregistration complete-----version: 1--");
	...
	//LOG.info("----------AvatarDataNode.startDataNode()-------444-----version: 1--");    
    ...
    if (simulatedFSDataset) {
      ...
      LOG.info("wzt debuginfo:----------AvatarDataNode.startDataNode()-------start read storage-----version: 1--");    	
      // read storage info, lock data dirs and transition fs state if necessary
      storage.recoverTransitionRead(nsInfo, dataDirs, startOpt);
      LOG.info("wzt debuginfo:----------AvatarDataNode.startDataNode()-------read storage complete,start init fsdataset-----version: 1--");
      // adjust
    ...
    LOG.info("wzt debuginfo:----------AvatarDataNode.startDataNode()-------fsdataset init complete,and start init threadgroup -----version: 1--");      
    // find free port
    ...
    LOG.info("wzt debuginfo: avatardatanode dataXceiverServer threadgroup start complete!!!");
    ...
    LOG.info("wzt debuginfo: avatardatanode start create info web servlet");
    String infoAddr = 
      NetUtils.getServerAddress(conf, 
    ...
    LOG.info("wzt debuginfo: avatardatanode info web servlet create complete!!!");
    ...
    //init ipc server
    LOG.info("wzt debuginfo: avatardatanode start create rpc ...");
    ...
    LOG.info("wzt debuginfo:avatardatanode rpc server create complete,dnRegistration = " + dnRegistration);
  }
  ...
  private synchronized NamespaceInfo handshake(boolean doWait) throws IOException {
    ...
    //LOG.info("--------------AvatarDataNode.handshake()----------111----------");
    do {
      try {
        if (namenode1 == null) {
          ...
          LOG.info("wzt testinfo:start nameAddr1:"+nameAddr1+" RPC.getProxy ");
        }
        if (avatarnode1 == null) {
          ...
          LOG.info("wzt testinfo:start avatarAddr1:"+avatarAddr1+" RPC.getProxy ");
        }
      ...
      try {
        if (namenode2 == null) {
          ...
          LOG.info("wzt testinfo:start nameAddr2:"+nameAddr2+" RPC.getProxy ");
        }
        if (avatarnode2 == null) {
          ...
          LOG.info("wzt testinfo:start avatarAddr2:"+avatarAddr2+" RPC.getProxy ");          
        }
  ...
  public void run() {
    ...
    //add by wzt 2013-12.25
    long[] ofThreadIDs = new long[2];
    //end add
    ...
    while (shouldRun) {
      try {
        ...
        if (namenode1 != null && !doneRegister1 &&
          ...
          doneRegister1 = true;
          //在这里检测of1线程的deadlock,ThreadInfo
        ...
        if(!offerService1.getOfferServiceRunning() 
        		&& (currentTime - offerService1.getLastHeartBeat()) > 1000*60){
       	    of1.interrupt();        		
        	of1 = null;
        	offerService1 = null;
            offerService1 = new OfferService(this, namenode1, nameAddr1, 
        ...
        if(!offerService2.getOfferServiceRunning() 
        		&& (currentTime - offerService2.getLastHeartBeat()) > 1000*60){
       	    of2.interrupt();        		
        	of2 = null;
        	offerService2 = null;
            offerService2 = new OfferService(this, namenode1, nameAddr2, 
                    avatarnode2, avatarAddr2);
            of2 = new Thread(offerService2, "OfferService2 " + nameAddr2);
            of2.start();
            ofThreadIDs[0] = of2.getId();
        }
        //end add
        ...
        if (blockScanner != null && blockScannerThread == null &&
          ...
          LOG.info("wzt debuginfo:avatardatanode start blockScannerThread ...");
        }
      } catch (Exception ex) {
