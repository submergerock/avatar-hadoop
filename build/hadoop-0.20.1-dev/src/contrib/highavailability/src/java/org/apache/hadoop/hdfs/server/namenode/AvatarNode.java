...
import com.dinglicom.com.server.extend.ProcessInternalMonitor;
...
public class AvatarNode extends NameNode implements AvatarProtocol {
  ...
  AvatarNode(Configuration startupConf, Configuration conf, Avatar avatar) throws IOException {
    ...
    //add by wzt 2013-12.26
	ProcessInternalMonitor processInfoMonitor = new ProcessInternalMonitor();
	Thread pm = new Thread(processInfoMonitor,"avatarnode processmonitor");
	pm.start();
    //end add
  }
  ...
  private void initialize(Configuration conf) throws IOException {
    ...
    LOG.info("wzt testinfo:avatarnode hdfs.avatarnode.handler.count:" + handlerCount);
    
    try{
    UserGroupInformation.setCurrentUser(UnixUserGroupInformation.login(conf));
    } catch (LoginException e){ }
    
    LOG.info("wzt testinfo:avatarnode RPC server  hostname:"+socAddr.getHostName()+
...
//added by wanglei --2011.11.16 --start
  public void copyEditLogAndFSImageToNFS( String NFSEditPath , String NFSImagePath )
  throws IOException {
  ...
  if (srcimage.exists()) {  
	  ...
	  if (destimage.exists()) {
	        ...
	        if (!destimage.renameTo(tmp)) {
	          throw new IOException("Unable to rename " + destimage +
	                                " to " +  tmp);
	        }
    ...
    if (!FileUtil.copy(localFs, new Path(srcimage.toString()), 
                      localFs, new Path(destimage.toString()), 
...
//    efile.delete();
  ...
  //add by wanglei,for dynamic nfs path
...
//added by wanglei --2012.2.11 --start
  public void copyFSImageToNFS(String NFSImagePath )
  throws IOException {
  String[] namedirs = this.startupConf.getStrings("dfs.name.dir");
  String msg = "";
  ...
  public void copyEditLogToNFS(String NFSEditPath)
  throws IOException {
  String[] editsdir = this.startupConf.getStrings("dfs.name.edits.dir");
  String msg = "";
...
//added by wanglei --2012.2.12 --start
  /**
