...
//import com.github.coderplay.util.concurrent.queue.ClaimStrategy;
//import com.github.coderplay.util.concurrent.queue.FastArrayBlockingQueue;
//import com.github.coderplay.util.concurrent.queue.MultiThreadedLowContentionClaimStrategy;
//import com.github.coderplay.util.concurrent.queue.MultiThreadedLowContentionWaitStrategy;
//import com.github.coderplay.util.concurrent.queue.WaitStrategy;
...
public abstract class Server {
  ...
  //update by wzt 2013-01-23
  //private BlockingQueue<Call> callQueue; // queued calls
  private BlockingQueue<Call>[] callQueueArray; // queued calls
  //private int MAX_CALLQUEUEARRAY = 8;
  //end by
  ...
  private class Listener extends Thread {
      ...
      if (force || numConnections > thresholdIdleConnections) {
        ...
        while (i <= end) {
          ...
          if (c.timedOut(currentTime)) {
        	//update by wzt 2013-12.25
            /*if (LOG.isDebugEnabled()){
                LOG.debug(getName() + ": disconnecting client " + c.getHostAddress());            	
            }*/
        	LOG.info("Server Call:"+getName() + ": disconnecting client " + c.getHostAddress());
        	//end add
            closeConnection(c);
            numNuked++;
            end--;
    ...
    public void run() {
      ...
      while (running) {
        ...
        try {
          ...
          if (running) {                          // unexpected -- log it
            LOG.info("Server Call " + getName() + " caught: " +
      ...
      LOG.info("Server Call Stopping " + this.getName());
    ...
    void doAccept(SelectionKey key) throws IOException,  OutOfMemoryError {
      ...
      for (int i=0; i<10; i++) {
        ...
        if (LOG.isDebugEnabled()){
            //update by wzt 2013-01-24
        	//LOG.debug("Server connection from " + c.toString() +
            //  "; # active connections: " + numConnections +
            //  "; # queued calls: " + callQueue.size());
        	LOG.debug("Server connection from " + c.toString() +
              "; # active connections: " + numConnections +
              "; # queued calls: " + getCallQueueLen());
        	
            //end by
        }//end fi
    	//LOG.info("Server connection from " + c.toString() +
        //        "; # active connections: " + numConnections +
        //        "; # queued calls: " + getCallQueueLen());
  ...
  private class Connection {
    ...
    //add by wzt 2013-01-24
    private void putCall(int callID,Call call) throws InterruptedException{
    	int index = callID % handlerCount;
    	callQueueArray[index].put(call);
    }
    //end by
  ...
  private class Handler extends Thread {
    ...
    public Handler(int instanceNumber) {
      this.setDaemon(true);
      this.threadID = instanceNumber;
      this.setName("IPC Server handler "+ instanceNumber + " on " + port);
    }

    //add by 2013-01-24
    private Call takeCall() throws InterruptedException{
    	return callQueueArray[this.threadID].take();
    }
    //end by
    @Override
    public void run() {
  ...
  protected Server(String bindAddress, int port, 
    ...
    this.callQueueArray = new LinkedBlockingQueue[handlerCount];
    for(int index=0;index < handlerCount;index++){
    	this.callQueueArray[index] = new LinkedBlockingQueue<Call>(maxQueueSize);
    }
    //ClaimStrategy claimStrategy = new MultiThreadedLowContentionClaimStrategy(8192);
    //WaitStrategy  waiStrategy   = new MultiThreadedLowContentionWaitStrategy();
    //this.callQueue = new FastArrayBlockingQueue<Call>(claimStrategy,waiStrategy);
    //end by 
  ...
  public synchronized void start() throws IOException {
    ...
    LOG.info("wzt testinfo:handlerCount:"+handlerCount);
  }
  ...
  public int getCallQueueLen() {
	  ...
	  int allCallQueueLen = 0;
	  for(int i=0;i<this.handlerCount;i++){
		  allCallQueueLen += this.callQueueArray[i].size();
      }
	  return allCallQueueLen;
    //end by
  }
