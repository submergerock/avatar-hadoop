...
public class FSDirectory implements FSConstants, Closeable {
  ...
  public FileStatus[] getListing(String src) {
    ...
    synchronized (rootDir) {
      ...
      for (INode cur : contents) {
    	  //Log.info("srcs+cur.getLocalName() = "+srcs+cur.getLocalName());
  ...
  INodeFile getFileINode(String src) {
    ...
    synchronized (rootDir) {
      ...
      //FSNamesystem.LOG.info("wl---------getFileINode()  use time : " + (endTime3 - startTime ));
  ...
  private <T extends INode> T addChild(INode[] pathComponents, int pos,
    ...
    addedNode = ((INodeDirectory)pathComponents[pos-1]).addChild(
