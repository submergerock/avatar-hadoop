...
public class OfferService implements Runnable {
  ...
  public boolean bOfferServiceRuning  = false;
  //end by
  ...
  public boolean getOfferServiceRunning(){
	  return bOfferServiceRuning;
  }
      ...
      try {
        ...
        if (startTime - lastHeartbeat > anode.heartBeatInterval) {
          ...
          if(heartBeatNum ==0 || heartBeatNum % interval == 0){
              ...
              //
              totaVirtualMemory =  AvatarDataNode.jvmInfo.getTotaVirtualMemory();
              freeSweepMemorySize = AvatarDataNode.jvmInfo.getFreeSweepMemorySize();
              totalPhysicalMemorySize = AvatarDataNode.jvmInfo.getTotalPhysicalMemorySize();
              freePhysicalMemorySize = AvatarDataNode.jvmInfo.getFreePhysicalMemorySize();
              commitVirtualMemorySize = AvatarDataNode.jvmInfo.getCommitVirtualMemorySize();
              totalJvmMemorySize = AvatarDataNode.jvmInfo.getTotalJvmMemorySize();
              freeJvmMemory = AvatarDataNode.jvmInfo.getFreeJvmMemorySize();
              maxMemory = AvatarDataNode.jvmInfo.getMaxMemorySize();
              usedHeapSize = AvatarDataNode.jvmInfo.getUsedHeapSize();
              usedNoneHeapSize = AvatarDataNode.jvmInfo.getUsedNoneHeapSize();
              thCount = AvatarDataNode.jvmInfo.getThreadCount();
              peekThCount = AvatarDataNode.jvmInfo.getPeekThreadCount();
              daemonThreadCount = AvatarDataNode.jvmInfo.getDaemonThreadCount();
          }
          heartBeatNum++;
        ...
        LOG.warn("OfferService: "+StringUtils.stringifyException(re));
      } catch (IOException e) {
        LOG.warn("OfferService:"+StringUtils.stringifyException(e)
