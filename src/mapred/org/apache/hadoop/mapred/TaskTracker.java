...
public class TaskTracker 
  ...
  //add by wzt 2012-12-14
  private long heartBeatNum = 0;
  private long heartBeatDUInterval = 2;
  //end ad
  ...
  synchronized void initialize() throws IOException {
    ...
    this.status = null;
    //add by wzt
    this.heartBeatDUInterval = this.fConf.getLong("mapred.tasktracker.du.interval", 2L);
    //end add
  ...
  private HeartbeatResponse transmitHeartBeat(long now) throws IOException {
    ...
    if (askForNewTask) {
      ...
      //update by wzt 2012-12-14,为了减少shell调用
      askForNewTask = enoughFreeSpace(localMinSpaceStart);
      freeDiskSpace = getFreeSpace();
      //if(this.heartBeatNum == 0 || this.heartBeatNum % this.heartBeatDUInterval == 0){
      //    askForNewTask = enoughFreeSpace(localMinSpaceStart);
      //    freeDiskSpace = getFreeSpace();
      //}
      //heartBeatNum++;
      //end add
      totVmem = getTotalVirtualMemoryOnTT();
      totPmem = getTotalPhysicalMemoryOnTT();
    ...
    LOG.info("wzt debuginfo:send heartbeat to jobtracker and getresponseid,askForNewTask="+askForNewTask
    		+" localMinSpaceStart="+localMinSpaceStart
    		+" freeDiskSpace="+freeDiskSpace
    		+" TotalVirtualMemoryOnTT="+totVmem
    		+" TotalPhysicalMemoryOnTT="+totPmem
      ...
      while (!Thread.interrupted()) {
        try {
          ...
          startNewTask(tip);
          	...
          	LOG.info("wzt debuginfo:tasktracker is outofmemory. Will retry in 30 seconds.", ie);
          	try {
