...
//---------add by wl826214---------2
  ...
  DataNode(Configuration conf, 
    ...
    this.conf = conf;
    try {
      LOG.info("---------before startDataNode!!!-----------");
      startDataNode(conf, dataDirs);
      LOG.info("---------after startDataNode!!!-----------");
    } catch (IOException ie) {
    	LOG.info("IOException Message: " + ie.getMessage());
    	LOG.info("IOException fillInStackTrace: " + ie.fillInStackTrace());
    	ie.printStackTrace();
    	shutdown();
      throw ie;
    }
  ...
  void startDataNode(Configuration conf, 
	...
	//LOG.info("------startDataNode---------000---------");
    if (conf.get("slave.host.name") != null) {
      machineName = conf.get("slave.host.name");   
    }
	//LOG.info("------startDataNode---------111---------");    
	...
	//LOG.info("------startDataNode---------333---------");    
    InetSocketAddress socAddr = NetUtils.createSocketAddr(address);
	//LOG.info("------startDataNode---------444---------");    
    int tmpPort = socAddr.getPort();
    storage = new DataStorage();
	//LOG.info("------startDataNode---------555---------");    
    ...
    if (simulatedFSDataset) {
      ...
      //-----wl826214-------
      //this.data.getBlockInputStream(b)
      //this.data.validateBlockFile
      //-----wl826214-------
    }
...
//  }
  ...
  public void offerService() throws Exception {
    ...
    while (shouldRun) {
      try {
        ...
        if (startTime - lastHeartbeat > heartBeatInterval) {
          ...
          LOG.info("wzt debuginfo: Just sent heartbeat, with name " + this.machineName
        ...
        if (blockArray != null) {
          ...
          LOG.info("wzt debuginfo: datanode send receive block list length="+blockArray.length
        ...
        if (startTime - lastBlockReport > blockReportInterval) {
          ...
          LOG.info("wzt debuginfo: datanode send all block report to nn ,block size="+bReport.length);
    ...
    LOG.error("wzt debuginfo:datanode offerService() exit,datanode is dead!!!");
  ...
  private void transferBlock( Block block, 
    ...
    if (numTargets > 0) {
      ...
      try{
      	...
      	LOG.info("wzt debuginfo: SocketTimeoutException ",ignored);
     }catch(AsynchronousCloseException ace){
         // another thread closed our listener socket - that's expected during
         // shutdown,but not in other circumstances
         if (shouldRun) {
           LOG.info("wzt debuginfo:AsynchronousCloseException "+this.dnRegistration + ":DataXceiverServer: ", ace);
         }
     }catch(OutOfMemoryError ie){
     	// DataNode can run out of memory if there is too many transfers.
     	// Log the event, Sleep for 30 seconds, other transfers may complete by then.
     	LOG.info("wzt debuginfo:DataNode transferBlock is outofmemory. Will retry in 30 seconds.", ie);
     	try {
  ...
  //ClientDataNodeProtocol implementation
  /** {@inheritDoc} */
//  public FSDatasetInterface getFSDataset() throws IOException{
//    return data;
//  }
 // IntWritable[] locationInvalidBlocks = null;
@Override
public String[]  getBlockFiles(Block[] b) {
	
	FSDataset fsds = null;
	if(this.data instanceof FSDataset)
		fsds = (FSDataset)this.data;		
	int numFile = 0;
	String[] s = new String[b.length];
	
	ArrayList<String> files = new ArrayList<String>();
	ArrayList<IntWritable> locationInvalidBlockArrayList = new ArrayList<IntWritable>();
	for(int i = 0 ;i < b.length ;i++){		
		try{			
			s[i] = fsds.getBlockFile(b[i]).getAbsolutePath();			
		} catch(IOException e){
			s[i] = "false";			
		}				
		//LOG.info("s["+i+"] : "+s[i]);
...
//	
//	
//	int numInvalidBlock = 0;
//	String[] s = new String[files.size()];
//	this.locationInvalidBlocks = new IntWritable[locationInvalidBlockArrayList.size()];
//	for(String file : files){
//		LOG.info("file : "+file);
//		s[numFile++] = file;
//	}
//	for(IntWritable bb : locationInvalidBlockArrayList){
//		LOG.info("locationInvalidBlock : "+bb);
//		
//		locationInvalidBlocks[numInvalidBlock++] = bb;
//	}

	
	
//	
//	ArrayWritable ArrayWritableS = new ArrayWritable(s);
//	ArrayWritable ArrayWritableInvalidb = new  ArrayWritable(IntWritable.class,locationInvalidBlock);
//	
//	ArrayWritable[] arrayWritables = new ArrayWritable[2];
//	arrayWritables[0] = ArrayWritableS;
//	arrayWritables[1] = ArrayWritableInvalidb;
//	
//	ArrayWritable arrayWritable = new ArrayWritable(ArrayWritable.class,arrayWritables);
	return s;
}

//	public String[]  getLocationInvalidBlocks() {
//		
//		IntWritable[] locationInvalidBlocks = this.locationInvalidBlocks;		
//		
//		return 
//	}
...
//---------add by wl826214---------6
private void startMyJob(Configuration conf) throws IOException {
	    this.myJob = new MyJob(this,conf);
	    this.myJobThread = new Thread(this.myJob);
	    this.myJobThread.setDaemon(true);
	    this.myJobThread.start();
}
//-------------wl826214------------2011.9.20--------
@Override
public void stopJob(Job j) throws IOException {
	//this.myJob.stopJob(j);
