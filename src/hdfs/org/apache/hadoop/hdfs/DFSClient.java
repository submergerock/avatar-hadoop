...
public class DFSClient implements FSConstants, java.io.Closeable {
  ...
  //begin ----------------------add by wanglei 2011.11.29 
  ...
  //begin ----------------------add by wanglei 2011.11.29  
    ...
    this.hdfsTimeout = conf.getInt("dfs.hdfs.timeout", 60 * 1000);//Client.getTimeout(conf);
  //end ----------------------add by wanglei 2011.11.29  
    try {
    ...
    LOG.info("dfsclient name:"+this.clientName);
  ...
  public OutputStream create(String src, 
	...
	LOG.info("create file "+src+ " ,is overwrite "+overwrite);
    ...
    //end ----------------------add by wanglei 2011.11.29  
      while (clientRunning && !Thread.interrupted()) {
        //if (System.currentTimeMillis() - lastRenewed > (LEASE_SOFTLIMIT_PERIOD / 2)) {
    	  if (System.currentTimeMillis() - lastRenewed > renewal) {//----------------------add by wanglei 2011.11.29  
          try {
            renew();
          ...
          //end ----------------------add by wanglei 2011.11.29  
            ...
            //LOG.warn("Problem renewing lease for " + clientName, ie);
              LOG.warn("Problem renewing lease for " + clientName +
		                       " for a period of " + (hdfsTimeout/1000) +
		                       " seconds. Will retry shortly...", ie);//----------------------add by wanglei 2011.11.29         	  
          }
        }
  ...
  ****************************************************************/
  class DFSOutputStream extends FSOutputSummer implements Syncable {
    ...
    public void setLastException(IOException e) {
    ...
    private class ResponseProcessor extends Thread {
      ...
      public void run() {
        ...
        while (!closed && clientRunning && !lastPacketInBlock) {
          ...
          //LOG.warn("----dataQueue.notifyAll() is end!!!-----");
          
          synchronized (ackQueue) {
            ackQueue.notifyAll();
          }
          
          //LOG.warn("----ackQueue.notifyAll() is end!!!-----");
    ...
    private LocatedBlock locateFollowingBlock(long start
      ...
      while (true) {
        ...
        while (true) {
          try {//begin---------added by wanglei---------2012.1.6
        	  if(DNIPs != null){
        		  return namenode.addBlockAssignDN(src, clientName, DNIPs);
        	  } //end---------addde by wanglei---------2012.1.6
    ...
    private synchronized void closeInternal() throws IOException {
      LOG.info("start closeInternal file:"+src+" ,come from "+clientName);
      checkOpen();
      isClosed();
...
//          LOG.debug("fileComplete : " + fileComplete); 
//          
//          LOG.debug("clientRunning : " + clientRunning); 
//          
//          LOG.debug("hdfsTimeout : " + hdfsTimeout); 
//          
//          LOG.debug("localstart + hdfsTimeout : " + (localstart + hdfsTimeout)); 
//          
...
//begin---------add by wanglei---------2011.12.4
  ...
  public String[] DNIPs = null;
 
  public void setDNIPs(String[] DNIPs)  {
	  if(DNIPs.length != this.defaultReplication){
		  try {
			  	LOG.info("DNIPs.length = " + DNIPs.length );
			  	
			  	for(String DNIP : DNIPs){
			  		LOG.info("DNIP = " + DNIP );
			  	}
			  	
				LOG.info("this.defaultReplication" + this.defaultReplication);
  ...
  public void recoverLease(String src) throws IOException{
  	synchronized(this){
