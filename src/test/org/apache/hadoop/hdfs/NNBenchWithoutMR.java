...
public class NNBenchWithoutMR extends Configured implements Tool {
  ...
  private static NNBenchThread handleThread = null;    
    ...
    if (singleFileExceptions >= maxExceptionsPerFile) {
        ...
        long duration = (endTime.getTime() - execTime.getTime()) /1000;
     	...
     	System.out.println("current total exceptions:"+singleFileExceptions+".Aborting"+
             " createfailfile:"+NNBenchThread.createfailFileCount+" closefailfile:"+NNBenchThread.closefailFileCount+
  ...
  static int createWrite() {
    ...
    for (int index = 0; index < numFiles; index++) {
      ...
      //System.out.println(" ***** create file #"+index);
      
      try{
    	  totalExceptions  = 5;
          do { // create file until is succeeds or max exceptions reached
              try {
                //update by wzt 2012-01-06
                //out = fileSys.create(
                //      new Path(taskDir, "" + index), false, 512,
                //      (short)1, bytesPerBlock);
                createFilePath = new Path(taskDir, "" + index);
                out = fileSys.create(
              		  createFilePath, false, 512,
           ...
           //System.out.println(" create file #"+createFilePath.toUri()+" success ");
           if(!success || totalExceptions <= 0){
        	     NNBenchThread.createfailFileCount ++;
                 System.out.println("create fail," +  NNBenchThread.createfailFileCount); 
                 continue;
            }
            ...
            if(index % 100 == 0){
                System.out.println(""+index+ " file"+createFilePath.toUri()+" create success, and waite close ...");            	
            }
            
            RemoteFileHandle fileHandle = new RemoteFileHandle(""+createFilePath.toUri(),out);
...
//            } while (!success);
  ...
  public int run(String[] args) throws Exception {
    ...
    handleThread = new NNBenchThread();
    handleThread.start();
    ...
    System.out.println("Fail create file count is "+NNBenchThread.createfailFileCount);
    System.out.println("Fail close  file count is "+NNBenchThread.closefailFileCount);
