package com.dinglicom.com.dfsclient;
...
public class TestdlDfsclient {
	//private static DFSClient dfsClient = null;
	private static DFSClient dfsClient = null;	
	private static byte[] buffer;
	private static String dfsTaskDir = "";
	private OutputStream out = null;
	private int totalindex = 0;
	private int fileLen = 0;
	private String createFilePath = null;
	
    public TestdlDfsclient(String dfsTaskDir,int index,int fileLen) throws IOException{
    	this.dfsTaskDir = dfsTaskDir;
    ...
    public void fillbuff(){
        ...
        System.arraycopy(sentence.toString().getBytes(), 0, buffer, 0, buffer.length);
    	
    }
    
    public void TestStartFile(int index) throws IOException{
    	...
    	do{
            ...
            this.createFilePath = createFilePath;
          	out = (OutputStream)this.dfsClient.create(createFilePath, 
          			...
          			4096);
              success = true;
              lbk = this.dfsClient.getBlockLocations(createFilePath, 0, 1024);
              System.out.println(this.createFilePath +" create success ...");
            } catch (IOException ioe) { 
              success=false; 
            }
    	}while(!success);
    	//System.out.println("lbk:"+lbk[0].getHosts()+" "+lbk[1].getHosts());
    }
    
    public void TestWriteFile(){
    	
    	BlockLocation[] lbk = null;
        long toBeWritten = this.fileLen;
        while (toBeWritten > 0) {
          int nbytes = (int) Math.min(buffer.length, toBeWritten);
          toBeWritten -= nbytes;
          try { // only try once
            out.write(buffer, 0, nbytes);
            lbk = this.dfsClient.getBlockLocations(this.createFilePath, 0, 1024);
          } catch (IOException ioe) {
    ...
    public void assembleFunc() throws IOException{
    	
    	for(int index = 0 ; index < totalindex ;index++){
    		this.TestStartFile(index);
    		this.TestWriteFile();
    		this.TestCloseFile();
    	}//end for
    }
	...
	public static void main(String[] args) throws IOException {
		...
		TestdlDfsclient testDfs = new TestdlDfsclient(args[0],Integer.parseInt(args[1]),Integer.parseInt(args[2]));
		testDfs.assembleFunc();
	}
